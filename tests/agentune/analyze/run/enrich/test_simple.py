...
@pytest.mark.asyncio
async def test_run_with_duplicate_names(run_context: RunContext, sample_dataset: Dataset) -> None:
    ...
    with pytest.raises(ValueError, match='Duplicate feature names found'), run_context.ddb_manager.cursor() as conn:
        await runner.run(features, sample_dataset, contexts, evaluators, conn, deduplicate_names=False)


@pytest.mark.asyncio
async def test_run_with_no_evaluator_for_feature(run_context: RunContext, sample_dataset: Dataset,
    ...
    with pytest.raises(ValueError, match='No evaluator found for features'), run_context.ddb_manager.cursor() as conn:
        await runner.run(sync_features, sample_dataset, contexts, evaluators, conn)

@pytest.mark.asyncio
async def test_run_stream(run_context: RunContext, sample_dataset: Dataset,
...
@pytest.mark.asyncio
async def test_empty_features_list(run_context: RunContext, sample_dataset: Dataset) -> None:
...
@pytest.mark.asyncio
async def test_larger_data_streaming(run_context: RunContext) -> None:
    with run_context.ddb_manager.cursor() as conn:
