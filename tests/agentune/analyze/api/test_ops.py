...
async def test_e2e_flow_synthetic(input_data_csv_path: Path, tmp_path: Path) -> None:
    async with await RunContext.create() as ctx:
        ...
        with pytest.raises(NoFeaturesFoundError): # Default generators can't do anything with this synthetic data
            await ctx.ops.search_for_features(problem_description, split_input)

        components = FeatureSearchComponents(
        ...
        results = await ctx.ops.search_for_features(problem_description, split_input, components=components)
...
async def test_feature_search_separate_test_input(input_data_csv_path: Path) -> None:
    async with await RunContext.create() as ctx:
        input = await ctx.data.from_csv(input_data_csv_path).copy_to_table('input')
        split_input = await input.split()
        problem_description = ProblemDescription('target')
        ...
        results1 = await ctx.ops.search_for_features(problem_description, split_input, components=components)
        ...
        for test_input in test_inputs:
            result = await ctx.ops.search_for_features(problem_description, split_input, test_input, components=components)
        ...
        for test_input_table in test_inputs_tables:
            result = await ctx.ops.search_for_features(problem_description, split_input, test_input_table, components=components)
            assert result.features_with_test_stats[0].stats.feature.n_total == custom_test.height
...
async def test_e2e_flow_real(test_data_conversations: dict[str, Path], tmp_path: Path) -> None:
    async with await RunContext.create() as ctx:
        ...
        results = await ctx.ops.search_for_features(problem_description, split_input, secondary_tables=[secondary], join_strategies=[join])
        assert len(results.features) > 0
