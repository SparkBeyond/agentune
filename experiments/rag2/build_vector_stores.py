#!/usr/bin/env python3
"""
Build and save persistent Chroma vector store for RAG2 simulation.
...
Requirements:
    ...
    - langchain-chroma for Chroma support
    
The script uses the same test data as the examples by default:
    - Data file: tests/data/dch2_sampled_dataset.json
    - Output: experiments/rag2/vector_store/chroma_db/
"""
...
from conversation_simulator.rag.indexing_and_retrieval import conversations_to_langchain_documents
...
DEFAULT_DATA_FILE = "dch2_sampled_dataset.json"
DEFAULT_DATA_DIR = Path("../../tests/data")  # Relative to script location
...
async def build_persistent_vector_store(
    ...
    """
    Build persistent Chroma vector store for RAG2 simulation.
    ...
    # Convert conversations to documents using the conversations_to_langchain_documents function
    # No role filtering - RAG2 uses metadata filtering during retrieval
    print("Converting conversations to documents...")
    documents = conversations_to_langchain_documents(
    ...
    print(f"   Store saved to: {persist_directory}")
